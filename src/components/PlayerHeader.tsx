...
export default function PlayerHeader({
  ...
  return (
    <section className="player-hero-shell overflow-hidden rounded-[1.65rem] border border-white/10 bg-[radial-gradient(circle_at_55%_0%,rgba(34,197,94,0.14),transparent_28%),linear-gradient(90deg,rgba(8,17,21,0.92),rgba(16,20,28,0.82))] p-6 text-white shadow-[0_24px_70px_rgba(0,0,0,0.32)] ring-1 ring-white/[0.04] sm:p-8">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          <p className="player-platform-label text-sm font-semibold text-blue-400 sm:text-base">{platform}</p>
            ...
            <div className="flex min-w-0 items-center gap-4">
              {visibleNationalityFlag ? (
                <span
                  ...
                  className="player-flag-shell flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.06] text-[2.35rem] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] sm:h-20 sm:w-20 sm:text-[3rem]"
                >
                  {visibleNationalityFlag}
                </span>
              ) : null}
        ...
        {comp ? (
          <div className="flex shrink-0 items-center gap-4 self-start md:self-center">
            <div className="player-comp-image h-24 w-20 overflow-hidden rounded-xl bg-white/[0.05] sm:h-32 sm:w-24">
              <img
